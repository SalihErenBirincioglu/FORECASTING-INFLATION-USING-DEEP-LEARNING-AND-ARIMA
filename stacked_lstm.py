# -*- coding: utf-8 -*-
"""
Created on Wed Oct 19 22:31:41 2022
...
while i<1:
    ...
    #print('Train Score for Stacked LSTM: %.2f RMSE' % (trainScore))
    
    testScore = math.sqrt(mean_squared_error(testY[0], testPredict[:,0]))
    print('Test Score Stacked LSTM: %.2f RMSE' % (testScore))
    AAtrain_rmse.append(trainScore)
    AAtest_rmse.append(testScore)
    
    # calculate mean absolute error
    trainMaeScore = mean_absolute_error(trainY[0], trainPredict[:,0])
    #print('Train Score using Mean Absolute Error for Stacked LSTM : %.2f MAE' % (trainMaeScore))
    
    testMaeScore = mean_absolute_error(testY[0], testPredict[:,0])
    print('Test Score using Mean Absolute Error for Stacked LSTM: %.2f MAE\n' % (testMaeScore))
